...
def registerUser(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request,"An error occur during registration. Try to use a stronger password")
...
def home(request):
    ...
    total_topics = Topic.objects.annotate(num_rooms=Count('room')).order_by('-num_rooms')
    topic_count =0 
    for topic in total_topics:
        topic_count += topic.room_set.all().count()
        
        
    topics = total_topics[0:5]
...
def createRoom(request):
    ...
    if request.method == 'POST':
        ...
        messages.success(request,"Create room successfully.")
        return redirect("home")
...
def updateRoom(request,pk):
    ...
    if request.method == 'POST':
        ...
        room.save()
        messages.success(request,"Your room has been updated.")
        return redirect("home") 
...
def deleteRoom(request,pk):
    ...
    if request.method == 'POST':
        ...
        if topic.room_set.all().count() <=0:
            topic.delete()
        messages.success(request,"Delete room successfully.")
        return redirect('home')       
    context = {'object':room}
...
def deleteMessage(request,pk):
    ...
    # room = message.room_set.all()
    if request.user != message.user:
        return HttpResponse("You are not allowed here!")
    
    if request.method == 'POST':
        message.delete()
        messages.success(request,"Delete message successfully.")
